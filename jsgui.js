function JSgui() {
    ...
    this.selectEquation = function (x) {
        this.currEq = x;
        $("#graph_inputs div.graph_input_wrapper").removeClass(
            "active_equation"
        );
    ...
    this.evaluate = function () {
        ...
        jsgcalc.draw();
        this.refreshInputs();
        //write here the falstad series
    };

    this.recursive = function () {
        jsgcalc.lines.push({
            equation: jsgcalc.lines[
    ...
    this.updateValues = function () {
        $("input.jsgcalc_xmin").val(
            Math.round(jsgcalc.currCoord.x1 * 1000) / 1000
        );
        $("input.jsgcalc_xmax").val(
            Math.round(jsgcalc.currCoord.x2 * 1000) / 1000
        );
        $("input.jsgcalc_ymin").val(
            Math.round(jsgcalc.currCoord.y1 * 1000) / 1000
        );
        $("input.jsgcalc_ymax").val(
            Math.round(jsgcalc.currCoord.y2 * 1000) / 1000
        );
    };
        ...
        for (i in equations) {
            $("#graph_inputs").append(
                '<div id="graph_input_wrapper_' +
                    ...
                    i +
                    '"></div>' +
                    '<div class="graph_equation_display"><span>f(x) =</span><input id="graph_input_' +
                    i +
                    '" size="20" value="' +
